...
// TestParseFormatter tests that parseFormatter returns formatter.Interface.
func TestParseFormatter(t *testing.T) {
	formatter := parseFormatter(testConfiguration.Loggers[0].Handlers[0].Formatter)
...
func TestGetLogger_Default(t *testing.T) {
	logger := GetLogger("notFound", *testConfiguration)
...
func TestGetAsyncLogger_Default(t *testing.T) {
	logger := GetAsyncLogger("notFound", *testConfiguration)
